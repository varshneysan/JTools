...
sub getNewOsubmitCandidate {
	...
	my $isDev;
        my $dbh = DBI->connect($dsn, $userid, $password ) or die $DBI::errstr;
	my $query = $dbh->prepare("SELECT Shelved_Change_No,Dependent_changes FROM sanity_check.cl_details where build_status='NEW' and Branch='//swdepot/main/' order by idEntries limit 1");
        $query->execute() or die $DBI::errstr;
        ...
        foreach (@row)
        {
            ...
            {
                push @changelists,$_;
		$query = $dbh->prepare("SELECT Branch FROM sanity_check.cl_details where Shelved_Change_No='$_'");
		$query->execute() or die $DBI::errstr;

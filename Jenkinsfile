...
node ("${Host}"){
    ...
    stage ('Synching WA') 
    {
		...
		sh 'if [ "${Branch}" ==  "main" ] || [ "${Branch}" == "sb-dcicls" ] || [ "${Branch}" ==  "r18.x" ] || [ "${Branch}" ==  "r18.0.x" ]; then p4 -u bangbuild -P ${P4PASSWD} -p ${P4PORT} -c ${P4CLIENT}  sync //swdepot/3dParty/NM/... > ${LOGS}/3dParty.log 2>&1 ;fi'
        }
		)
